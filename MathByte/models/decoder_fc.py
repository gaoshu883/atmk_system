...
class DecoderFC(nn.Module):
    def __init__(self, encoder_hidden_size, output_size, fc_layer_sizes,
        ...
        self.module_list.append(
            nn.Linear(encoder_hidden_size * self.num_direction, fc_layer_sizes[0]))
        self.module_list.append(nn.BatchNorm1d(
            fc_layer_sizes[0], momentum=0.5))
        ...
        for i in range(len(fc_layer_sizes) - 1):
            self.module_list.append(
                nn.Linear(fc_layer_sizes[i], fc_layer_sizes[i+1]))
            self.module_list.append(nn.BatchNorm1d(
                fc_layer_sizes[i+1], momentum=0.5))
            self.module_list.append(nn.LeakyReLU())
            self.module_list.append(nn.Dropout(self.dropout_p))
        # convert to hidden size in order to use attention
        self.module_list.append(
            nn.Linear(fc_layer_sizes[-1], self.hidden_size * self.num_direction))
        ...
        self.fc_final = nn.Linear(
            self.hidden_size * self.num_direction, output_size)
