...
class DecoderRNN(nn.Module):
    ...
    def __init__(self, vocab_size, max_len, hidden_size, sos_id, eos_id, loss_type,
        ...
        self.rnn = self.rnn_cell(self.hidden_size, self.hidden_size,
        ...
        if use_attention:
            self.attention = LabelAttention(self.hidden_size)
    ...
    def forward(self, inputs=None,
        ...
        for di in range(max_length-1):
            ...
            decoder_output, symbols = self.decoder(
                context, mask, candidates, logit_output=logit_output)
            ...
            if teacher_forcing_ratio < 1.0:
                ...
                step_symbols = (is_ss * sample_symbols.float() +
            ...
            if 'candidates' in self.decoder.sampling_type:
                candidates -= utils.to_one_hot(symbols.squeeze(
                    2).squeeze(1), self.output_size).float()
